...
def getCameraPosition(size, lon, lat, heading, pitch, fov, key):
      ...
      data['key'] = key
      url_metadata = 'https://maps.googleapis.com/maps/api/streetview/metadata?' + parse.urlencode(data)
      print(url_metadata)
      url =       'https://maps.googleapis.com/maps/api/streetview?' + parse.urlencode(data)
...
# third party code that detects the pole in the pciture and makes sure that the whole pole is in the image.
...
def storeCameraPos(camera_pos, heading_from_camera, directory):
    ...
    camerafilename = directory.decode('utf-8') + '/' + storefilename+ 'camera.json'
...
def processPole(poleName, poleData):
    ...
    if camera_pos['status']=="OK":
        heading_from_camera = angleFromCoordinate(centerLon, centerLat, camera_pos['location']['lng'], camera_pos['location']['lat'])
        # start with a zoomed in image to detect small poles
        polepos = downloadStreetViewImage(size, centerLon, centerLat, heading_from_camera, pitch,  80, key, directory, storefilename)
    ...
    (polepos['pole']['top']['x'] < BottomX)):
        # zoom only
        fov = ScalePole( (polepos['pole']['bottom']['y']-polepos['pole']['top']['y']), min(polepos['pole']['top']['y'], 2000-polepos['pole']['bottom']['y']))
        polepos = downloadStreetViewImage(size, centerLon, centerLat, heading_from_camera, pitch,  fov, key, directory, storefilename)
        storeCameraPos(camera_pos, heading_from_camera, directory)
        return
    ...
    (polepos['pole']['top']['x'] <TopX or (polepos['pole']['top']['x'] >BottomX))
        ...
        camera_pos = getCameraPosition(size, new_pos['lon'], new_pos['lat'], heading, pitch, 90, key) #When looping zoom out
        if camera_pos['status']=="OK":
            heading_from_camera = angleFromCoordinate(centerLon, centerLat, camera_pos['location']['lng'], camera_pos['location']['lat'])
            polepos = downloadStreetViewImage(size, new_pos['lon'], new_pos['lat'], heading_from_camera, pitch,  90, key, directory, storefilename) #When looping zoom out
            ...
            if (polepos['pole']['top']['y'] > PoleTop) and (polepos['pole']['bottom']['y'] <PoleBottom):
                ...
                fov = ScalePole( (polepos['pole']['bottom']['y']-polepos['pole']['top']['y']),min(polepos['pole']['top']['y'], 2000-polepos['pole']['bottom']['y']))
                print(fov)
                polepos = downloadStreetViewImage(size, new_pos['lon'], new_pos['lat'], heading_from_camera, pitch,  fov, key, directory, storefilename)
                storeCameraPos(camera_pos, heading_from_camera, directory)
            storeCameraPos(camera_pos, heading_from_camera, directory)
        else:
            print("No Camera Picture")
    ...
    if camera_pos['status']=="OK":
        heading_from_camera = angleFromCoordinate(centerLon, centerLat, camera_pos['location']['lng'], camera_pos['location']['lat'])
        # start with a zoomed in image to detect small poles
        polepos = downloadStreetViewImage(size, centerLon, centerLat, heading_from_camera, pitch,  100, key, directory, storefilename)
        storeCameraPos(camera_pos, heading_from_camera, directory)
    else:
        print("No Camera Picture")
    if ((polepos['status'] != 0) and 
    ...
    (polepos['pole']['top']['x'] <TopX or (polepos['pole']['top']['x'] >BottomX))
        ...
        if camera_pos['status']=="OK":
            heading_from_camera = angleFromCoordinate(centerLon, centerLat, camera_pos['location']['lng'], camera_pos['location']['lat'])
            polepos = downloadStreetViewImage(size, new_pos['lon'], new_pos['lat'], heading_from_camera, pitch,  90, key, directory, storefilename)
            print(polepos)
            storeCameraPos(camera_pos, heading_from_camera, directory)    
        else:
            print("No Camera Picture")
